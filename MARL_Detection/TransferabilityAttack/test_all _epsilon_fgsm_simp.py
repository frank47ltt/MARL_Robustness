...
binary_model = load_model('../BinaryNets/LSTMSimpWhitePredict_ZS_WB.keras')

for i in range(1,21):
    eps = np.round((i*0.05),2)*100
    testX = np.load('fgsm_test_simpX_ZS_WB' + str(eps) + '.npy')
    testY = np.load('fgsm_test_simpY_ZS_WB'+ str(eps) + '.npy')
    trueActions = np.load('fgsm_simp_trueActions_ZS_WB'+ str(eps) + '.npy')
    print(testX.shape)
    print(testY.shape)
