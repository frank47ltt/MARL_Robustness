...
binary_model = load_model('../BinaryNets/LSTMStarWhitePredict_CF_BB.keras')

for i in range(1,21):
    eps = np.round((i*0.05),2)*100
    testX = np.load('fgsm_test_starX_CF_BB' + str(eps) + '.npy')
    testY = np.load('fgsm_test_starY_CF_BB'+ str(eps) + '.npy')
    trueActions = np.load('fgsm_star_trueActions_CF_BB'+ str(eps) + '.npy')
    print(testX.shape)
    print(testY.shape)
