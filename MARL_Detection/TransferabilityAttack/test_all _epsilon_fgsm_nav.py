...
for i in range(1,21):
    eps = 0.05 * i * 100
    testX = np.load('fgsm_testX_ZS_WB' + str(eps) + '.npy')
    testY = np.load('fgsm_testY_ZS_WB' + str(eps) + '.npy')
    trueActions = np.load('fgsm_trueActions_ZS_wB' + str(eps) + '.npy')
    print(testX.shape)
    print(testY.shape)
    ...
    print("FN (success rate of fooling model with epsilon" + str(eps) + "):  " + str(float(matrix[1][0])/(float(matrixA[1][0])+float(matrixA[1][1]))))
