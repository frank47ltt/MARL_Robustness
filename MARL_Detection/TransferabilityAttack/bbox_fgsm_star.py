...
model = load_model('ReplicaStarEnsembleLogits_CF_BB.keras')
...
for j in range(1,21):
    ...
    np.save("fgsm_test_starX_CF_BB" + str(np.round(epsilon,2) * 100),attacked_X)
    print(trueActions.shape)
    print(np.unique(trueActions))
    np.save("fgsm_test_starY_CF_BB"+ str(np.round(epsilon,2) * 100),newY)
    np.save("fgsm_star_trueActions_CF_BB"+ str(np.round(epsilon,2) * 100),trueActions)