...
import sys
#sys.path.insert(0,"/Applications/StarCraft II/")
from Maps import get_map_params
#sys.path.insert(0,"/root/qmix/env")
...
class StarCraft2Env(MultiAgentEnv):
    ...
    def step(self, actions):
        ...
        # print("actions_int is " + str(actions_int))
        ...
        for a_id, action in enumerate(actions_int):
            if not self.heuristic_ai:
                ...
                # print("sc action is " + str(sc_action))
            else:
    ...
    def get_agent_action(self, a_id, action):
        ...
        # print("avail_action is " + str(avail_actions))
    ...
    def get_avail_agent_actions(self, agent_id):
        ...
        unit = self.get_unit_by_id(agent_id)
        # print("unit is " + str(unit) + "with agent id " + str(agent_id))
        if unit.health > 0:
    ...
    def get_unit_by_id(self, a_id):
        """Get unit by ID."""
        # print("here" + str(self.agents[a_id]))
    ...
    def update_world(self, real_env):
        self._episode_steps = real_env._episode_steps
        # Information kept for counting the reward
        self.death_tracker_ally = real_env.death_tracker_ally
        self.death_tracker_enemy = real_env.death_tracker_enemy
        self.previous_ally_units = real_env.previous_ally_units
        self.previous_enemy_units = real_env.previous_enemy_units
        self.win_counted = real_env.win_counted
        self.defeat_counted = real_env.defeat_counted

        self.last_action = real_env.last_action

        if self.heuristic_ai:
            self.heuristic_targets = real_env.heuristic_targets

        try:
            self._obs = real_env._obs
        except (protocol.ProtocolError, protocol.ConnectionError):
            self.full_restart()
