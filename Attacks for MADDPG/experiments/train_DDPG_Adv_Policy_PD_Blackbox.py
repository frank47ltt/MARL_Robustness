...
def parse_args():
    ...
    parser.add_argument("--save-dir", type=str, default="./weights_save/",
...
def train(arglist):
    with U.single_threaded_session() as marl_sess:
        ...
        if collect_AIRL:
            ...
            file_obs_good = open("PD_Obs_Good.txt", "w")
            file_act_good_disc = open("PD_Act_Good_dist.txt", "w")

        coop_dist = 0
        avg_coop_dist = []

        D = Discriminator("./disc_saved_model", (10,), 5)  # initialize the AIRL discriminator
        ...
        if loaded:
            print("loading-------------------------------------------------------------")
            saver_DDPG.restore(marl_sess, './bb_weights_DDPG_Adv_PD_final/')
        ...
        expert_observations = np.genfromtxt('AIRL data/PD_Obs_Good.txt')
        expert_actions = np.genfromtxt('AIRL data/PD_Act_Good_dist.txt', dtype=np.int32)
        ...
        while True:
            ...
            if terminal and (len(episode_rewards) % arglist.save_rate == 0):
                ...
                if not loaded:
                    saver_DDPG.save(marl_sess, './bb_weights_DDPG_Adv_PD_save/')
